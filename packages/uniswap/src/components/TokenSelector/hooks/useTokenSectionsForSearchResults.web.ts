// Web-specific version with on-chain fallback
// This uses createEthersProvider from packages so it can be used in packages directory
...
// Import the web-specific search hook with fallback
// Vite will resolve this at build time using the alias
type UseSearchTokensWithFallbackType = (args: {
...
let useSearchTokensWithFallbackWeb: UseSearchTokensWithFallbackType | null = null

function getWebSearchHook() {
  if (typeof window === 'undefined') {
    return null
  }

  // Try to import the web-specific hook
  // Vite alias 'apps/web/src' should resolve this
  try {
    // @ts-ignore - Dynamic import for web-specific code, resolved by Vite alias
    const module = require('apps/web/src/hooks/useSearchTokensWithFallback')
    useSearchTokensWithFallbackWeb = module.useSearchTokensWithFallback
    return useSearchTokensWithFallbackWeb
  } catch (_e) {
    return null
  }
}
...
export function useTokenSectionsForSearchResults({
  ...
  // Try to use web-specific hook with fallback, otherwise use generic
  const webSearchHook = getWebSearchHook()
  const useSearchHook = webSearchHook || require('uniswap/src/features/dataApi/searchTokens').useSearchTokens

  // Use the fallback hook that includes on-chain fetching
  const {
  ...
  const [selectedNetworkResults, otherNetworksSearchResults] = useMemo((): [CurrencyInfo[], CurrencyInfo[]] => {
    ...
    const selected = searchResultCurrencies.filter((currency: CurrencyInfo) => !currency.isFromOtherNetwork)
    const other = searchResultCurrencies.filter((currency: CurrencyInfo) => currency.isFromOtherNetwork)
