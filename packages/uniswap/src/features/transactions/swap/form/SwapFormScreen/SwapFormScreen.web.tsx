        ...
        {process.env.NODE_ENV !== 'production' && (
          ...
          >
            <Text variant="body3" color="$neutral1">
              [DEBUG] SwapFormScreen.web.tsx is rendering. Restricted tokens:{' '}
              {allowlistChecks.debug.restrictedTokens.length}, Blocking: {allowlistChecks.blockingWarnings.length},
              Non-blocking: {allowlistChecks.nonBlockingWarnings.length}, Loading: {isLoading ? 'true' : 'false'},
              Checks: {allowlistChecks.debug.checked.length}, Account: {accountAddress ? 'yes' : 'no'}, Chain:{' '}
              {chainId || 'none'}, InputToken: {inputToken?.symbol || 'none'}, OutputToken:{' '}
              {outputToken?.symbol || 'none'}
            </Text>
          </Flex>
        )}
